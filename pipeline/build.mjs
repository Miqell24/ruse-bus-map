...
// Ruse: ONE feed, and this map writes it itself — see pipeline/ruse-feed.mjs.
// 19 bus lines in the family's navy and 7 trolleybus lines in its green, both
// on the road graph. No tram: Ruse has never had one.
...
// Exact-match repairs for poles a feed names badly. Ruse needs none: every
// stop name here IS an OSM name — ruse-feed.mjs geocoded the operator's
// timetable against the OSM poles, so what the feed carries is what the
// mappers wrote.
const NAME_FIX = {};
...
// the operator's own timetable pages (transport-ruse.com), geocoding their
// stop NAMES against the OSM poles in the frame. So this feed has stops and
// stop sequences but NO shapes, and the engine draws it the way it draws
// Olsztyn and Grodzisk: the stop chain is the observation chain and the
// routing between poles is the geometry.
//
// LINE KEYS. The operator writes T for a trolleybus line and A for a bus one
// — T2, A3 — but the vehicle and the stop flag show the bare number, and the
// two sets never collide (trolleybuses 2, 9, 13, 21, 24, 27, 29; buses 3…166).
// So the mode letter stays in the KEY, where it keeps the two families apart
// and traceable back to the timetable page, and LBL prints the number the
